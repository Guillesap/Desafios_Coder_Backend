...
export default class CartContainer {
  ...
  async addProductToCartById(id, product) {
    try {
      ...
      return returnMessage(false, "Película agregada al carrito", cart);
    } catch (error) {
      return returnMessage(
        true,
        "Error al agregar película  al carrito",
        null
      );
    }
  }
  ...
  async deleteProductFromCartById(idCart, idProduct) {
    try {
      ...
      if (!cart.products.find((product) => product.id === idProduct)) {
        return returnMessage(true, "La Película no existe", null);
      }
      ...
      return returnMessage(false, "Película  eliminada del carrito", cart);
    } catch (error) {
      return returnMessage(
        true,
        "Error al eliminar la película del carrito",
        null
      );
    }
  }
  ...
  async getById(id) {
    try {
      ...
      return returnMessage(true, "Error al obtener el Carrito de compras", null);
    }
  }
